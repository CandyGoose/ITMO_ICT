'''
программа использует модуль aiohttp для захвата верхних постов в Reddit и вывода их на консоль

Требуется установить содуль
pip install aiohttp
'''
import sys 
import asyncio 
...
async def get_reddit_top(subreddit, client, numposts): 
    data = await get_json(client, 'https://www.reddit.com/r/' + 
        subreddit + '/top.json?sort=top&t=day&limit=' + str(numposts))
    print(f'\n/r/{subreddit}:')
